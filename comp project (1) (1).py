...
def donate():
    ...
    name_of_donar = input("name of donar(full name): ")
    name_of_parent_or_gardian_of_donar = input("name of any one parent or gardien: ")
    current_address = input("enter your current address: ")
    city = input("enter city/ district: ")
    ...
    age_of_donar = int(input("age: "))
    gender = input("sex: ")
    blood_group = input("blood group: ")
    emergency_phno = input("emergency contact number: ")
    mobile(emergency_phno)
    emergency_address = input("emergency contact address: ")
    print("choose any one of the identity proofs'\n' 1.PAN card '\n' 2.voter id '\n' 3.Driving license '\n' 4.Aadhae card")
    type_of_proof = input("enter identity proof: ")
    proof_no = input("enter the above identity proof number: ")
    print("choose the organ u would like to donate : \n'all organs' \n 'corneas(eyes)'\n' kidneys'\n' heart'\n' lungs'\n' liver'\n' pancreas '\n 'small intestine'\n' skin'. ")
    organ_type = input("enter the organ you would like to donate: ")
    age(age_of_donar)
    print("hope you have verified your details \n")
    print(" IT TAKES A GREAT HEART TO BE AN ORGAN DONOR. \n WE SOLEMNLY WISH YOU GOOD")
    tname='ORGAN_DONATE'
    Organ_Donate_Record_Add(neme_of_donar,name_of_parent_or_gardian_of_donar,current_address,city,state,pin_code,ph_no,occupation,email_id,dob,age_donar,gender,blood_group,emergency_phno,emergency_address,proof_no,organ_type)    
    Print_Record_Table_Form(tname)
def req():
    print(" please enter the following details of the patient correctly so that we can help ")
    tname="ORGAN_REQUEST"
    name_req = input("name of the person who requires an organ: ")
    age_req = int(input("age: "))
    phno_req = input("contact number: ")
    mobile(phno_req)
    email_req = input("email.id: ")
    name_hospital_req = input("name of the hospital: ")
    hosp_branch_req = input("branch: ")
    hosp_phno_req = input("hospital phone nnumber: ")
    hpn(hosp_phno_req)
    city_req = input("city/district where the patient is admitted: ")
    state_req = input("state: ")
    bloodgrp_req = input("blood group: ")
    name_organ_req = input("please enter the name of the organ required: ")
    ...
    Organ_Request_Record_Add(name_req,age_req,phno_req,email_req,name_hospital_req,hosp_branch_req,hosp_phno_req,city_req,state_req,bloodgrp_req,name_organ_req)
    Print_Record_Table_Form(tname)
